...
async function writeHistoryOperationBlinkit(blinkitData) {
  ...
  for (const row of blinkitData) {
    ...
    try {
      ...
      if (existingRows[0].count === 0) {
        console.log(`   ⚠️ No base record for EAN ${ean}, skipping`);
        skipCount++;
        continue;
      }

      // 3️⃣ INSERT new snapshot (copy last row)
      const [insertResult] = await historyDb.query(
        `INSERT INTO sku_inventory_report (
          ...
          marketplace_speed_7_days, marketplace_speed_15_days, marketplace_speed_30_days,
          instamart_stock, instamart_speed, instamart_day_cover,
          zepto_stock, zepto_speed, zepto_days_of_cover,

          blinkit_b2b_stock, blinkit_b2b_speed, blinkit_b2b_days_of_cover,
          blinkit_marketplace_stock, blinkit_marketplace_speed, blinkit_marketplace_days_of_cover,

          blinkit_b2b_drr_7d, blinkit_b2b_drr_15d, blinkit_b2b_drr_30d,

          quickcomm_speed_7_days, quickcomm_speed_15_days, quickcomm_speed_30_days,
          bigbasket_stock, bigbasket_speed, bigbasket_days_of_cover,
          purple_stock, purple_speed, purple_days_of_cover,
          reliance_stock, reliance_speed, reliance_days_of_cover,
          nykaa_stock, nykaa_speed, nykaa_days_of_cover,
          meesho_stock, meesho_speed, meesho_days_of_cover,
          ...
          Zepto_B2B_drr_7d, Zepto_B2B_drr_15d, Zepto_B2B_drr_30d,
          created_at
        )
        SELECT
          brand, gb_sku, asin, multiple_listing, ean_code, product_title, category,
  ...
  console.log(`📊 Total: ${blinkitData.length}`);
